...
export function CardGenerator(flags: HSLRandomFlags = {}): string {
  ...
  const h = flags.hueRandom
    ? Math.floor(Math.random() * 360)
    : (flags.hue ?? 0);
  const s = flags.saturationRandom
    ? Math.floor(Math.random() * 101)
    : (flags.saturation ?? 0);
  const l = flags.lightnessRandom
    ? Math.floor(Math.random() * 101)
    : (flags.lightness ?? 0);
...
export function CardGeneratorRandomHue(
  s: number,
  l: number,
  count: number,
): string[] {
  ...
  let hslColor = HSLColorGenerator({
